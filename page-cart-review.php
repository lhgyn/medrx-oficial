...
<main>
	...
	<div class="container">
		...
		<div class="col-4 ml-auto">
			...
			<?php foreach ( $WC->cart->get_cart_contents() as $key => $val ):
				...
				?>
					<div class="d-flex flex-row justify-content-between align-items-stretch border-bottom">
						<div>
							<img src="<?= wp_get_attachment_image_url( $val['data']->image_id, 'large' ) ?>" class="img-fluid" alt="" style="max-width: 100px">
						</div>
						
						<div class="d-flex flex-column">
							<p class="text-right"><b><?=$val['data']->name?></b></p>
							<p class="text-right">Qtd: <?=$val['quantity']?> &#8680; <?php echo apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $_product, $val['quantity'] ), $val, $key ); ?></p>
						</div>
					</div>
			<?php endforeach; ?>
			...
			<p class="border-bottom mt-2" style="display:<?= $WC->cart->shipping_total > 0?'block':'none';   ?>">
				<span class="float-left">Frete</span>
				<span class="float-right">R$<?= number_format($WC->cart->shipping_total, 2, ',', '.') ?></span>
			</p>
			<p class="mt-2">
